...
public class PutAction : IPlayerAction
{
    ...
    void IPlayerAction.Execute(string[] inputTextArray)
    {
        ...
        ActionUtil.MatchZeroOneAndMany<IItem>(
            items,
            () => StoryTextHandler.invokeUpdateTextDisplay("You can't put that"),
            item =>
            {
                ...
                ActionUtil.MatchZeroOneAndMany<IContainer>(
                    containers,
                    () => StoryTextHandler.invokeUpdateTextDisplay("You can't put into that"),
                    container =>
                    {
                        switch (itemLocation)
                        {
                        ...
                        StoryTextHandler.invokeUpdateTextDisplay($"You put {item.referenceName} in {container.referenceName}");
