...
public class Criterion
{
    ...
    public static Fact FindFirstFact(IEnumerable<Fact> facts, RuleKey key)
    {
        return facts.FirstOrDefault(f => f.key == key);
    }

    public static List<Fact> FindAllFacts(IEnumerable<Fact> facts, RuleKey key)
    {
        return facts.Where(f => f.key == key).ToList();
    }

    public static bool FactValueGreaterThan<T>(IEnumerable<Fact> facts, RuleKey key, T value) where T : IComparable
    {
        Fact fact = FindFirstFact(facts, key);
        return fact != null && fact.value is T typedValue && typedValue.CompareTo(value) > 0;
    }

    public static bool FactValueLessThan<T>(IEnumerable<Fact> facts, RuleKey key, T value) where T : IComparable
    {
        Fact fact = FindFirstFact(facts, key);
        return fact != null && fact.value is T typedValue && typedValue.CompareTo(value) < 0;
    }

    public static bool FactValueGreaterThanOrEqual<T>(IEnumerable<Fact> facts, RuleKey key, T value) where T : IComparable
    {
        Fact fact = FindFirstFact(facts, key);
        return fact != null && fact.value is T typedValue && typedValue.CompareTo(value) >= 0;
    }

    public static bool FactValueLessThanOrEqual<T>(IEnumerable<Fact> facts, RuleKey key, T value) where T : IComparable
    {
        Fact fact = FindFirstFact(facts, key);
        return fact != null && fact.value is T typedValue && typedValue.CompareTo(value) <= 0;
    }

    public static bool FactValueNotEquals(IEnumerable<Fact> facts, RuleKey key, object value)
    {
        Fact fact = FindFirstFact(facts, key);
    ...
    public static bool FactDoesNotExist(IEnumerable<Fact> facts, RuleKey key, object value)
    {
    ...
    public static bool FactExists(IEnumerable<Fact> facts, RuleKey key, object value)
    {
        List<Fact> matchingFacts = FindAllFacts(facts, key);
        Fact fact = matchingFacts.FirstOrDefault(fact => fact.value.Equals(value));
        return fact != null;
